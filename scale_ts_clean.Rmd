...
```{r}
## data preparation
df_pt = s_ts %>% filter(id== i) %>% 
      mutate(date_ym = paste(year(date), month(date), sep = "-")) %>% 
      arrange(date) %>% 
      distinct()

df_pt = replace_outliers(df_pt)
df_pt['qtyLog'] = log(df_pt$quantity+1)

df_prophet2 = df_pt %>% dplyr::select(c(date, qtyLog)) %>%
rename(ds = date, y = qtyLog)

weeks_train = round(as.numeric(difftime(max(df_pt$date), min(df_pt$date), unit="weeks"))*0.7)
split_date = min(df_pt$date) + weeks(weeks_train)  
...
for (k in seq_len(nrow(all_params))) {
  ...
  # Cross-validation
  df_cv <- cross_validation(m, initial = round(as.numeric(difftime(split_date, min(df_pt$date), 
          units = "days"))), period = 30, horizon = 30, units = 'days')
  # Model performance
