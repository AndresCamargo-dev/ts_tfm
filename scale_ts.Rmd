...
for (i in id_ts){
  ...
  df = s_ts %>% filter(id== i) %>% 
      ...
      geom_line(aes(y = quantity, color = "Actual")) +
      geom_line(aes(y = qtyProphetTun, color = "Prophet")) +
      ...
      auto_model <- prophet(df_prophet2, changepoint.prior.scale = best_params$changepoint_prior_scale, #0.05
        ...
        ## Cross-validation
      auto_model_cv <- cross_validation(auto_model, initial = round(as.numeric(difftime(split_date_70,  min(df$date), units = "days"))), period = 30, horizon = 30, units = 'days')
...
round(as.numeric(difftime(split_date,  min(df_prophet2$ds), units = "days")))
