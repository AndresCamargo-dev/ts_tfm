...
smapes <- numeric(length = nrow(all_params))
...
for (i in seq_len(nrow(all_params))) {
  ...
  smapes[i] <- df_p[['smape']][1]
}

# Tuning results
tuning_results <- data.frame(all_params, smape = smapes)
# Find the best parameters
best_params <- all_params[which.min(smapes), ]
print(best_params)
